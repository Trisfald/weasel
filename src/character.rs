...
impl<R: BattleRules> std::fmt::Debug for AlterStatistics<R> {
...
impl<R: BattleRules + 'static> Event<R> for AlterStatistics<R> {
    fn verify(&self, battle: &Battle<R>) -> WeaselResult<(), R> {
        // Check if this entity can accept alterations on statistics.
        if !self.id.is_character() {
            return Err(WeaselError::NotACharacter(self.id.clone()));
        }
